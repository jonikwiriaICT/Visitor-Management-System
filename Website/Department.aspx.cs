...
public partial class Department : System.Web.UI.Page
{
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateData(currentPage, "GetDepartment");
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                ...
                if (itrig == 0)
                {
                    if (objAdm.CRUDDepartment(rec_id.Value , department_name.Text , department_code .Text , "", "INSERT") == true)
                    {
                        LoadGrid();
                ...
                if (itrig == 1)
                {
                    if (objAdm.CRUDDepartment(rec_id.Value, department_name.Text, department_code.Text, "", "UPDATE") == true)
                    {
                        LoadGrid();
                        DisplaySuccess(objAdm.ErrorMessage);
